# TODO - B-ALSC features not yet implemented

This example implements **as much of the B-ALSC (Advanced Life Safety
Controller) profile as the standard CAS BACnet Stack's customer-facing API
supports today**. B-ALSC is B-LSC plus an Event Log and internal scheduling;
this example is seeded from `BACnetProfileExample-B-LSC-CPP`'s `implement-lsc`
branch (PR open, not yet merged there at the time this file was written), so
items 0-2 below are carried forward from B-LSC's own TODO.md verbatim - they
are the SAME stack-level gaps, re-verified against this repository's own
build, not re-discovered or re-filed as a new issue. Item 3 is new to this
repository (F-EVENTLOG). The items below are the parts of B-ALSC that are
**not** implemented here, why, and what it would take. They are revisited as
the stack gains the capability.
...
runbook's "touch ONLY BACnetProfileExample-B-ALSC-CPP" instruction).
...
## 3. Event Log 1 "Beige" cannot demonstrate capturing Amber's/Azure's own alarm notifications - same root cause as item 0

**Status: verified by running this repository's own built binary against a
live `bacpypes3` client. Everything about Beige that is INDEPENDENT of item
0's Life Safety defect is verified working; only the alarm-capture demo is
blocked, by the identical root cause, not a new one.**

### What works (verified over the wire)

- `BACnetStack_AddEventLogObject(deviceInstance, 1, 100)` creates the object;
  `Object_Name` ("Beige"), `Object_Type` (`event-log`), `Buffer_Size` (100),
  `Record_Count` and `Total_Record_Count` (both 0 at start-up) all read back
  correctly.
- `Enable` (`Log_Enable`, property id 133) is writable: `WriteProperty` to
  `false` then `true` both succeed and read back correctly.
- `ReadRange` of `Log_Buffer` (service 35, `range_params=("p", 1, None, None,
  10)`) succeeds - per `BACnetStack_AddEventLogObject`'s own doc comment,
  ReadProperty of `Log_Buffer` is refused, but ReadRange is the correct way to
  read it and does so.
- The stack's automatic capture hook is REAL, not a fiction of the doc
  comment: toggling `Enable` false->true (a clause-12.27-mandated log-status
  transition) was independently captured with no application code involved -
  `Record_Count`/`Total_Record_Count` both moved from 0 to 1 and `ReadRange`
  returned exactly one `EventLogRecord` afterward. This confirms Beige's
  underlying capture engine (sprint-112's hook, per
  `submodules/cas-bacnet-stack/docs/sprints/sprint-112-event-log-engine.md`)
  is live in this build, not merely documented.

### What is blocked, and why

The profile card's intended demo - Amber's or Azure's own CHANGE_OF_LIFE_SAFETY
notification (routed through Crimson) showing up as a captured Log_Buffer
record - cannot be exercised. `BACnetStack_AddEventLogObject`'s own doc comment
says Log_Buffer records "normally accumulate from received event
notifications" (an automatic capture of the device's own and/or received
notifications, not an application call - `BACnetStack_InsertEventLogRecord`,
the only app-facing way to insert one directly, was moved to the test-tool
surface in v6 and is forbidden by this series). So a captured alarm record
requires Amber/Azure to actually reach `alarm(2)` and have Crimson send a
notification - and that is exactly the path item 0 (issue #2036) blocks: the
only trigger this file offers, a `WriteProperty` of `Present_Value`, fails
with `unknown-object` before it ever reaches this file's callbacks or (as far
as can be observed from the customer surface) the stack's own intrinsic
algorithm. Reconfirmed live in this repository's own build: after the failed
`WriteProperty` attempt, `Record_Count`/`Total_Record_Count` stayed at
whatever they were before the attempt (0, in a clean run) - no alarm, no
notification, nothing for Beige to capture.

This is not a new or independent gap - it is item 0's consequence for the one
new object this repository adds over B-LSC. Once issue #2036 is fixed (stack
exports `BACnetStack_AddLifeSafetyPointObject`/`AddLifeSafetyZoneObject`),
this item resolves automatically: no change to Beige's own configuration is
needed, only to Amber/Azure's (see item 0's "To do when available").

**Stack issue:** none filed separately - this is downstream of
https://github.com/chipkin/cas-bacnet-stack/issues/2036, referenced there.
